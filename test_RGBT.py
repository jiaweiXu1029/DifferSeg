...
from sam2unet_RGBT import SAM2
from dataset_RGBT import TestDataset
...
parser.add_argument("--thermal_folder", type=str, default="thermal",
                    help="folder name for thermal maps in each dataset")
...
for item in os.listdir(args.data_root):
    ...
    if os.path.isdir(dataset_path):
        ...
        has_thermals = os.path.exists(os.path.join(dataset_path, args.thermal_folder))
        has_masks = os.path.exists(os.path.join(dataset_path, args.mask_folder))

        if has_images and has_thermals and has_masks:
...
for dataset_name in datasets:
    ...
    test_thermal_path = os.path.join(args.data_root, dataset_name, args.thermal_folder) + "/"
    ...
    # 加载当前数据集
    test_loader = TestDataset(test_image_path, test_thermal_path, test_gt_path, 512)
    ...
    for i in range(test_loader.size):
        with torch.no_grad():
            image, thermal, gt, name = test_loader.load_data()
            gt = np.asarray(gt, np.float32)
            image = image.to(device)
            thermal = thermal.to(device)
            res, _, _ ,_= model(image, thermal)
