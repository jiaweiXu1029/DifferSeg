...
from dataset_RGBD import FullDataset
...
parser.add_argument("--val_depth_path", type=str, default=None,
                    help="path to the depth images used for validation")
...
def main(args):
    ...
    # 训练集
    train_dataset = FullDataset(args.train_image_path, args.train_depth_path, args.train_mask_path, 512, mode='train')
    ...
    if args.val_dir and os.path.isdir(args.val_dir):
        ...
        for dataset in datasets:
            ...
            if os.path.isdir(dataset_path):
                ...
                # 检查所有必要的文件夹是否存在
                if os.path.isdir(image_path) and os.path.isdir(depth_path) and os.path.isdir(mask_path):
                    try:
                        val_dataset = FullDataset(image_path, depth_path, mask_path, 512, mode='val')
                    ...
                    print(f"Skipping dataset '{dataset}' - missing required folders. Found: "
                          f"image:{os.path.isdir(image_path)}, depth:{os.path.isdir(depth_path)}, mask:{os.path.isdir(mask_path)}")

    # 如果没有找到验证集或使用传统方式
    if not val_dataloaders and args.val_image_path and args.val_depth_path and args.val_mask_path:
        val_dataset = FullDataset(args.val_image_path, args.val_depth_path, args.val_mask_path, 512, mode='val')
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, batch in enumerate(tqdm(train_dataloader, desc="Training")):
            ...
            pred0, pred1, pred2, pred3 = model(x, depth)
        ...
        if val_dataloaders:
            ...
            checkpoint_path_latest = os.path.join(args.save_path, 'SAM2-UNet-RGBD-latest.pth')
            torch.save({
                'epoch': epoch,
            ...
            if avg_mae < best_val_mae:
                ...
                checkpoint_path_best = os.path.join(args.save_path, 'SAM2-UNet-RGBD-best_mae.pth')
                torch.save({
                    'epoch': epoch,
                    'model_state_dict': model.state_dict(),
                    'optimizer_state_dict': optim.state_dict(),
